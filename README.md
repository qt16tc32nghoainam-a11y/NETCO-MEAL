...
- API chạy dưới dạng Serverless Function tại `api/index.ts`. File này tạo một app Express tối giản và gắn lại **chính** `apiRouter` từ `server/api.ts` (không nhân bản logic route). Default export là một handler dạng `function handler(req, res) { return app(req, res); }` — vì một app Express vốn đã là một handler `(req, res)` — đây là dạng tương thích rộng nhất với runtime `@vercel/node`. Kèm middleware bắt lỗi trả JSON để tránh lỗi 500 mờ mịt.
- **Gắn router phòng thủ 2 lớp:** `apiRouter` được mount ở **cả** `/api/v1` **và** `/`. Tùy phiên bản runtime, đường dẫn mà Vercel chuyển vào function có thể còn giữ tiền tố `/api/v1` hoặc đã bị cắt bỏ; mount ở cả hai base đảm bảo `/auth/login` luôn khớp đúng `apiRouter.post('/auth/login')` trong mọi trường hợp — loại bỏ nguyên nhân phổ biến nhất gây 404/500 do lệch đường dẫn.
- **Module system (điểm mấu chốt để hết lỗi 500):** function được biên dịch **CommonJS** (`api/tsconfig.json`: `module: CommonJS`, `moduleResolution: Node`, `esModuleInterop`) — tức file `api/index.js` sau biên dịch dùng `exports` / `Object.defineProperty(exports, ...)`. NHƯNG `package.json` **gốc** có `"type": "module"`, nên Node coi **mọi** file `.js` dưới `/var/task` (kể cả `api/index.js`) là **ES module**; một ESM không có biến `exports`, dẫn tới lỗi `ReferenceError: exports is not defined in ES module scope` ngay khi nạp function → HTTP 500 ở mọi route. **Cách sửa:** thêm một `api/package.json` lồng nhau với đúng nội dung `{ "type": "commonjs" }`. Một `package.json` lồng nhau **ghi đè** trường `"type"` cho các file **chỉ trong thư mục `api/`**, nên `api/index.js` (CommonJS) được nạp đúng như CommonJS, trong khi gốc dự án vẫn giữ ESM để Vite hoạt động bình thường. Nhờ vậy giữ được CommonJS cho toàn bộ import quan hệ không-đuôi trong chuỗi server (`../server/api` → `./db` → `../src/types`) — nếu ép sang ESM (NodeNext) thì Node sẽ yêu cầu thêm đuôi `.js` cho mọi import này và function sẽ lỗi phân giải. tsconfig riêng này cũng để function không kế thừa các thiết lập chỉ dành cho Vite ở tsconfig gốc (`allowImportingTsExtensions`, `noEmit`, `moduleResolution: bundler`) vốn có thể khiến function lỗi.
- **Không thêm dependency mới:** giữ nguyên `@types/express` sẵn có thay vì thêm `@vercel/node`. Runtime `@vercel/node` vẫn gọi default export theo dạng `handler(req, res)` bất kể kiểu TypeScript, nên không cần thêm gói; tránh rủi ro lệch `bun.lock` gây lỗi cài đặt lúc build trên Vercel.
- **Zero-config (để Vercel tự nhận diện & build function `api/`):** `vercel.json` **KHÔNG** khai báo `buildCommand` hay `outputDirectory`. Đây là điểm mấu chốt: khi ép sẵn `buildCommand: "vite build"` + `outputDirectory: "dist"`, Vercel coi dự án là **thuần tĩnh (static/SPA)** và **không** phát hiện / biên dịch / đóng gói serverless function trong thư mục `api/` — dẫn tới rewrite `/api/v1/*` trỏ tới một function không tồn tại trong build output và gọi API bị HTTP 500. Bỏ hai khai báo đó đi thì Vercel bật **zero-config**: tự nhận diện framework **Vite** (chạy `vite build` → xuất `dist/` mặc định) **đồng thời** tự build mọi file `api/*.ts` thành **Node serverless function**. Nhờ vậy `api/index.ts` được phục vụ tại đường dẫn `/api/index`.
- `vercel.json` chỉ giữ `rewrites`: mọi lời gọi `/api/v1/:path*` được chuyển tới function `api/index` (**đặt TRƯỚC** catch-all SPA để không bao giờ rewrite `/api/*` về `index.html`); các đường dẫn còn lại (không thuộc `api/`) fallback về `index.html` để React SPA (định tuyến phía client, refresh trang) hoạt động.
- Frontend được build bằng Vite (`vite build`, do Vercel tự chạy nhờ nhận diện framework) và phục vụ tĩnh từ `dist/`.
- **Luồng đầu-cuối:** trình duyệt `POST /api/v1/auth/login` → `vercel.json` rewrite `/api/v1/:path*` → function `api/index` (do Vercel zero-config build từ `api/index.ts`) → Express (`express.json` parse body) → `apiRouter` (mount ở `/api/v1` và `/`) → handler `/auth/login` → trả JSON 200 `{ data: { user, accessToken, ... } }`.
...
Lưu ý: script `npm run build` trong `package.json` vẫn giữ nguyên (build frontend + đóng gói `server.ts` bằng esbuild) để phục vụ chạy standalone/local (`npm run start`) và các nền tảng khác (ví dụ Render). Trên Vercel thì **không** dùng script này: nhờ zero-config, Vercel tự nhận diện Vite để build frontend và tự đóng gói function `api/*.ts`, nên `vercel.json` không cần khai báo `buildCommand`/`outputDirectory`.

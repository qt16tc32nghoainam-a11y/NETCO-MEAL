...
- **Module system:** function được biên dịch **CommonJS** (`api/tsconfig.json`: `module: CommonJS`, `moduleResolution: Node`, `esModuleInterop`). Toàn bộ import quan hệ trong chuỗi server (`../server/api` → `./db` → `../src/types`) đều **không có đuôi `.js`**; nếu ép sang ESM (NodeNext) thì Node sẽ yêu cầu thêm đuôi `.js` cho mọi import này và function sẽ lỗi lúc chạy. CommonJS phân giải import không-đuôi một cách tự nhiên nên là lựa chọn an toàn, chuẩn mực. tsconfig riêng này cũng để function không kế thừa các thiết lập chỉ dành cho Vite ở tsconfig gốc (`allowImportingTsExtensions`, `noEmit`, `moduleResolution: bundler`) vốn có thể khiến function lỗi.
